...
AMI_ID="ami-063dd2bd5641ef903"
echo $AMI_ID 

create_server() {

    echo -n "$COMPONENT server creation is in progress"
    PRIVATE_IP=$(aws ec2 run-instances --image-id $AMI_ID --instance-type t3.micro --security-group-id $SGID --tag-specifications "ResourceType=instance,Tags=
    [{Key=Name,Value=${COMPONENT}-${ENV}}]"  | jq '.Instances[].PrivateIpAddress' | sed -e 's/"//g')
